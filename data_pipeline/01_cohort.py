...
# Config
MIN_LOS_HOURS  = 24     # minimum ICU stay length
MIN_AGE        = 18     # may need to modify later
READMIT_WINDOW = 30     # days for readmission label


def build_cohort(con: duckdb.DuckDBPyConnection) -> pd.DataFrame:
    """
    Core cohort = one row per ICU stay.
    Joins: icustays + admissions + patients
    Labels: in-hospital death, 30-day readmission
    """
    ...
    -- 30-day readmission: does the same patient have another admission
...
def add_diagnoses(cohort: pd.DataFrame, con: duckdb.DuckDBPyConnection) -> pd.DataFrame:
    """Attach primary ICD diagnosis code to each stay."""
